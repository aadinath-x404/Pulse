...
function initHome() {
  ...
  function render() {
    ...
    for (const ev of filtered) {
      ...
      card.className = 'p-4 rounded-md bg-neutral-800 border border-neutral-700 flex items-start justify-between transition-colors duration-150 hover:bg-neutral-700';
      card.innerHTML = `
        <div>
        ...
        <div class="text-neutral-400 text-sm">
          <button data-id="${ev.id}" class="deleteBtn px-2 py-1 rounded-md hover:bg-neutral-700 transition-colors duration-150" aria-label="Delete event">Delete</button>
        </div>`;
...
function initCalendar() {
  ...
  function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const cell = document.createElement('div');
      cell.className = 'h-24 p-2 border border-neutral-800 rounded-md flex flex-col justify-between hover:bg-neutral-800 transition-colors duration-150';
